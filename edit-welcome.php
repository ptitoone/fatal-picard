...
function addNews($message, $author)
{
    $db = Database::connect();
    $statement = $db->prepare('INSERT INTO news (message, author, date) VALUES (?, ?, NOW())');
    $statement->execute(array($message, $author));
    Database::disconect();
}
...
if (isset($_POST['message']) && $admin) {
    ...
    addNews($message, $_SESSION['pseudo']);
}
?>
    ...
    <div class="container-fluid wrapper-out">
        <div class="container login">
            ...
            <div class="row">
                <div class="col">
                    <?php 
                    if ($success && isset($_POST['message']) && $admin) {
                        echo '<h2>Message d\'acceuil à jour avec succès !</h2>';
                    }
                    else {
